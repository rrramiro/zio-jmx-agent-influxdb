...
import fr.ramiro.influxdbclient.model._
...
class PointSuite extends CustomTestSuite {
  ...
  test("BigDecimal fields are serialized correctly") {
    assert(
      BigDecimalField("key", BigDecimal("51.98890310")).serialize == "key=51.98890310"
    )
  }

  test("Fields are escaped correctly") {
    assert(
      StringField("ke y", "a v=al\"ue").serialize == "ke\\ y=\"a v=al\\\"ue\""
    )
    assert(LongField("key,", 12123L).serialize == "key\\,=12123i")
  }
}
