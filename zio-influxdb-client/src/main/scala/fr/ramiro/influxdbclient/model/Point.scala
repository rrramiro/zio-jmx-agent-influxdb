...
case class Point(
  ...
  def addField(key: String, value: BigDecimal) =
    copy(fields = BigDecimalField(key, value) +: fields)

  def serialize = {
    val sb = new StringBuilder //TODO refacto
...
case class SeqPoint(points: Seq[Point]) extends InfluxSerializable {
  def serialize: String = points.map(_.serialize).mkString("\n")
}
