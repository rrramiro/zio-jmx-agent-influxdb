...
import fr.ramiro.influxdbclient.{IBoolean, IString, InfluxDB}
import fr.ramiro.influxdbclient.{IBoolean, IString, InfluxDB}

protected[influxdbclient] trait UserOperations { self: InfluxDB =>
  def createUser(
      username: String,
      password: String,
    ...
    var queryString =
      "CREATE USER %s WITH PASSWORD '%s'".format(username, password)
    if (isClusterAdmin)
  ...
  def userIsClusterAdmin(username: String) = {
    showUsers().map(
      result =>
        result.series.head.records.exists(
          record =>
            record("user") == IString(username) && record("admin") == IBoolean(
              true
            )
        )
    )
